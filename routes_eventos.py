...
def verificar_evento_aberto_existente(tipo='NORMAL'):
    ...
    Retorna o evento existente ou None.
    """
    try:
        query = """
        ...
        # Se a coluna tipo não existe, retorna qualquer evento aberto
        query = """
    ...
    if result:
        return EventoResponse(
            ...
            tipo=result[5] if len(result) > 5 else "NORMAL"
        )
...
async def listar_eventos_ativos(
    ...
    # Usamos datetime.now() para garantir que a comparação seja feita com o horário da aplicação
    # e não do banco de dados (que pode estar em outro fuso)
    query = """
        SELECT id, nome, data_evento, status, data_limite, data_criacao, tipo
        FROM eventos
        WHERE status = 'ABERTO' AND data_limite > :current_time
    ...
    # Se usuário não é admin, filtrar eventos
    if not current_user.get("is_admin"):
        usuario_id = current_user["id"]
        eventos_permitidos = []
        
        for evento in eventos:
            if not evento.get("TIPO") or evento["TIPO"] == "NORMAL":
                eventos_permitidos.append(evento)
            else: # Tipo 'RELAMPAGO' ou outro tipo restrito
                acesso_query = """
                    SELECT 1 FROM evento_acessos
                    WHERE evento_id = :evento_id AND usuario_id = :usuario_id
                """
                tem_acesso = execute_query(
                    acesso_query,
                    {"evento_id": evento["ID"], "usuario_id": usuario_id},
                    fetch_one=True
                )
                if tem_acesso:
                    eventos_permitidos.append(evento)
...
async def obter_evento_ativo(
    ...
    """Obtém o evento NORMAL atualmente aberto para pedidos (compatibilidade)"""
    
    # Fechar eventos expirados automaticamente
    verificar_e_fechar_eventos_expirados()
    
    # Prioriza o evento mais próximo (menor data_evento) que ainda está aberto
    # Tenta primeiro com a coluna tipo, se falhar usa query sem tipo
    try:
        query = """
            SELECT id, data_evento, status, data_limite, data_criacao, tipo
            FROM eventos
        ...
        # Se a coluna tipo não existe, usa query sem ela
        query = """
            SELECT id, data_evento, status, data_limite, data_criacao
            FROM eventos
            WHERE status = 'ABERTO' 
...
async def criar_evento(
    ...
    tipo_evento = evento.tipo if evento.tipo else 'NORMAL'
    
    # Verificar se já existe evento aberto do mesmo tipo
    if tipo_evento == 'NORMAL':
        evento_existente = verificar_evento_aberto_existente(tipo='NORMAL')
        if evento_existente:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Já existe um evento NORMAL aberto (Pizzada de {evento_existente.data_evento.strftime('%d/%m/%Y')}). Feche o evento atual antes de criar um novo."
            )
    ...
    with get_db_connection() as conn:
        ...
        # Buscar ID do evento criado (Oracle não retorna lastrowid facilmente em INSERT)
        cursor.execute(
        ...
        # Se for relâmpago e tiver usuários permitidos
        if tipo_evento == 'RELAMPAGO' and evento.allowed_users:
            for uid in evento.allowed_users:
                try:
                    cursor.execute(
                        "INSERT INTO evento_acessos (evento_id, usuario_id) VALUES (:eid, :uid)",
                        {"eid": evento_id, "uid": uid}
                    )
                except Exception as e:
                    print(f"Erro ao adicionar acesso para user {uid}: {e}")
    ...
    return EventoResponse(
        ...
        tipo=result[6] if len(result) > 6 else "NORMAL"
    )
...
async def atualizar_evento(
    ...
    check_query = "SELECT id, tipo FROM eventos WHERE id = :evento_id"
    ...
    evento_tipo = existing[1] if len(existing) > 1 else 'NORMAL'
    ...
    if evento.status is not None:
        # Se tentando reabrir um evento NORMAL, verificar se já existe outro aberto
        if evento.status == 'ABERTO' and evento_tipo == 'NORMAL':
            evento_existente = verificar_evento_aberto_existente(tipo='NORMAL')
            if evento_existente and evento_existente.id != evento_id:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Não é possível reabrir este evento. Já existe outro evento NORMAL aberto (Pizzada de {evento_existente.data_evento.strftime('%d/%m/%Y')})."
                )
...
async def obter_resumo_evento(
    ...
    return ResumoEvento(
        evento=EventoResponse(
            ...
            tipo=evento_result[5] if len(evento_result) > 5 else "NORMAL"
        ),
