...
@router.get("/ranking")
async def ranking_sabores(
    current_user: dict = Depends(get_current_user)
):
    """Ranking global de sabores mais pedidos"""
    
    query = """
        SELECT sp.id, sp.nome, sp.tipo,
        ...
        LEFT JOIN itens_pedido ip ON ip.sabor_id = sp.id
...
async def obter_sabor(
    ...
    return SaborPizzaResponse(
        id=result["ID"],
        nome=result["NOME"],
        preco_pedaco=float(result["PRECO_PEDACO"]),
        ativo=bool(result["ATIVO"]),
        data_cadastro=result["DATA_CADASTRO"],
        tipo=result.get("TIPO", "SALGADA"),
